    ...
    def __init__(self,
        ...
        if if_compress:
            self.avgpool = nn.Sequential(
                nn.ZeroPad2d(pooling_kernel // 2),
                nn.AvgPool2d(pooling_kernel, 1)
            )

    def forward(self, x):
        x = x if not self.if_compress else self.avgpool(x)
        weight = self.conv1(x)
        ...
        # print('3', weight.shape)
        out = _involution_cuda(x, weight, stride=1, padding=(self.kernel_size - 1) // 2)
        # print('4', out.shape)
        return out
...
class Residual(nn.Module):  # pytorch
    def __init__(self,
                 in_channels, out_channels, kernel_size, use_1x1conv=False, if_compress=True):
        super(Residual, self).__init__()
        self.conv1 = nn.Sequential(
            involution(in_channels, kernel_size),
        ...
        #if use_1x1conv:
         #   self.conv3 = nn.Sequential(nn.Conv2d(out_channels, in_channels, kernel_size=1, stride=1),
          #                             nn.BatchNorm2d(in_channels),
           #                            nn.ReLU(inplace=True)
            #                           )
        #else:
...
class reSSRN(nn.Module):
    def __init__(self, channel, classes):
        super(reSSRN, self).__init__()
        self.conv2d_1 = nn.Conv2d(channel, 200, kernel_size=1)
        self.batch_norm1 = nn.Sequential(
            nn.BatchNorm2d(200),
            nn.ReLU(inplace=True))
        #self.ca1 = ChannelAttention(200, 16)
        #self.sa1 = SpatialAttention(5)
        self.res_net1 = Residual(200, 200, 3, use_1x1conv=False)
        self.res_net2 = Residual(200, 200, 3, use_1x1conv=False)
        self.res_net3 = Residual(100, 100, 3, use_1x1conv=False)
        # self.res_net4 = Residual(100, 100, 5, use_1x1conv=False)
        ...
        #self.ca2 = ChannelAttention(100, 16)
    ...
    def forward(self, x):
        x = self.batch_norm1(self.conv2d_1(x))
        #x = self.ca1(x) * x
        #x = self.sa1(x) * x
        # print('x1', x1.shape)
...
if __name__ == '__main__':
    input = torch.randn(2, 200, 9, 9).cuda()
    inv = involution(200, 3, if_compress=True).cuda()
    # out = inv(input)
    # print(out.shape)
    flops, params = get_model_complexity_info(inv, (200, 9, 9), as_strings=True, print_per_layer_stat=True)
